<template>
  <div class="training">
      ...
      <el-col :span="24">
        ...
        <el-card class="table-content">
          <el-table :data="userList" border style="width: 100%">
            ...
            <el-table-column align="center" prop="address" label="培训记录"></el-table-column>
        ...
        <el-table-column label="操作" fixed="right" align="center" width="200">
          <template slot-scope="scope">
            ...
            <el-popconfirm title="此经历内容确定永远删除吗？">
              <el-button style="margin-left:10px" type="danger" size="mini" icon="el-icon-delete" slot="reference"></el-button>
            </el-popconfirm>
          </template>
        </el-table-column>
      </el-table>
    ...
    >
      <!-- 表单内容 -->
      <el-form :model="addTrainingForm" ref="addTrainingForm" label-width="120px" class="trainings">
        <div
          class="addFormContent"
          v-for="(item, index) in addTrainingForm.addTranings"
          :key="index"
        >
          <el-row>
            <el-col :span="2" align="center" style="height: 200px;line-height: 200px">
              <el-tag>{{index}}</el-tag>
            </el-col>
            <el-col :span="19">
              <el-row>
                <el-col :span="24">
                  <el-form-item
                    label="培训时间"
                    :prop="'addTranings.' + index + '.start_end'"
                    :inline-message="true"
                    :rules="[{ required: true, message: '请输入日期', trigger: 'blur' }]"
                  >
                    <el-date-picker
                      v-model="item.start_end"
                      type="daterange"
                      range-separator="至"
              ...
              <el-row>
                <el-form-item
                  ...
                  class="address"
                  label="地址"
                  :prop="'addTranings.' + index + '.address'"
                  :rules="{ required: true, message: '请输入省市级', trigger: 'blur' }"
                >
                  <el-cascader size="mini" :props="{}" v-model="item.address"></el-cascader>
                </el-form-item>
                <el-form-item
                  :inline-message="true"
                  prop="detail_address"
                  :rules="{ required: true, message: '请输详细地址', trigger: 'blur' }"
                >
                  <el-input
                ...
                <el-form-item
                  :inline-message="true"
                  label="培训项目"
                  :prop="'addTranings.' + index + '.training_program'"
                  :rules="{ required: true, message: '请输入培训项', trigger: 'blur' }"
                >
                  <el-select size="mini" v-model="item.training_program" placeholder="请选择">
                ...
                <el-form-item
                  :inline-message="true"
                  label="培训内容"
                  :prop="'addTranings.' + index + '.content'"
            ...
            <el-col :span="3" style="height: 200px;line-height: 200px">
              <el-button type="danger" size="mini" @click="deleteFormBtn(index)">删除</el-button>
            </el-col>
          </el-row>
        </div>
      </el-form>
      <!-- 增加 -->
      <el-row>
        <el-col :span="24" class="addTrainingFormCol">
          <el-button
            class="addTrainingForm"
            icon="el-icon-plus"
            size="mini"
            @click="addTrainingFormBtn"
          ></el-button>
        </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    // 添加培训记录表单
    addTrainingFormBtn() {
      this.addTrainingForm.addTranings.push({
        start_end: [],
        address: [],
        detail_address: "",
        training_program: "",
    ...
    addTrainingFormClose() {
      // 重置（只保留一个）
      this.addTrainingForm.addTranings.splice(
        1,
        this.addTrainingForm.addTranings.length
      );
      // 重置表单
